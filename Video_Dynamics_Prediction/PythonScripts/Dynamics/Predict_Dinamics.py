    ...
    with torch.no_grad():
        ...
        while(i<max_len_test):
            ...
            if(i+batch_size)>max_len_test:
                ...
                prev_z = torch.tensor(test_z[i:,0].reshape((batch_size, 1, z_size)), dtype=torch.float32)
            else:
                prev_z = torch.tensor(test_z[i:i+batch_size,0].reshape((batch_size, 1, z_size)), dtype=torch.float32)

            for j in range(1, frames):

                X = test_p_h_v[i:i+batch_size, j].reshape((batch_size, 1, img_size))
                y = test_z[i:i+batch_size, j].reshape((batch_size, 1, z_size))
                ...
                out = model(input_video, prev_z, flag)
    ...
    with torch.no_grad():
        ...
        while(i<max_len_test):
            ...
            if(i+batch_size)>max_len_test:
                ...
                X = test_p_h_v[i:i+batch_size]
                y = test_z[i:i+batch_size]

            # Step 2. Prepare inputs
            input_video = torch.tensor(X[:, 1:], dtype=torch.float32)
            prev_z = torch.tensor(y[:, :-1], dtype=torch.float32)
            ...
            # print(targets.size())
            # Step 3. Run our forward pass.
            out = model(input_video, prev_z, True)
...
def re_Train(model, optimizer, loss_func, train_p_h_v, train_z, val_p_h_v, val_z, out_dir, batch_size, epochs):
    ...
    for epoch in range(epochs):  
        ...
        while(i<max_len_train):
            ...
            if(i+batch_size)>max_len_train:
                ...
                prev_z = torch.tensor(train_z[i:,0].reshape((new_batch_size, 1, z_size)), dtype=torch.float32)
            else:
                new_batch_size = batch_size
                prev_z = torch.tensor(train_z[i:i+new_batch_size,0].reshape((new_batch_size, 1, z_size)), dtype=torch.float32)
            ...
            for j in range(1, frames):
                ...
                # Step 2. Prepare inputs
                X = train_p_h_v[i:i+new_batch_size, j].reshape((new_batch_size, 1, img_size))
                y = train_z[i:i+new_batch_size, j].reshape((new_batch_size, 1, z_size))
        ...
        if val_loss < min_loss : 
            torch.save(model, f"{out_dir}\\dynamics_model_retrained.pt")
            Flag = True
            min_loss = val_loss
            best_epoch = epoch
    ...
    if Flag :
        ...
        print(f"Loading the best model from {out_dir}\\dynamics_model_retrained.pt")
        model = torch.load(f"{out_dir}\\dynamics_model_retrained.pt")
        print(model)
...
def Train(model, optimizer, loss_func, train_p_h_v, train_z, val_p_h_v, val_z, out_dir, batch_size, epochs):
    ...
    for epoch in range(epochs):  
        ...
        while(i<max_len_train):
            ...
            if(i+batch_size)>max_len_train:
                ...
                X = train_p_h_v[i:i+batch_size]
                y = train_z[i:i+batch_size]
        ...
        if val_loss < min_loss : 
            torch.save(model, f"{out_dir}\\dynamics_model.pt")
            Flag = True
            min_loss = val_loss
            best_epoch = epoch
            print("\n\nNew min_loss found : ", min_loss)
    ...
    if Flag :
        print("\n\n\n Minimum val loss found at epoch : ", best_epoch+1)
        print(f"Loading the best model from {out_dir}\\dynamics_model.pt")
        model = torch.load(f"{out_dir}\\dynamics_model.pt")
        print(model)
...
def main(args):
    ...
    if args.re_train:
        ...
        if args.option=="LSTM":
            model = LSTM(embed_dim=train_p_h_v.shape[2], output_dim=train_z.shape[2], hidden_dim=20, n_layers=1, drop_prob=0.5)
        elif args.option=="FF":
            model = FF(embed_dim=train_p_h_v.shape[2], output_dim=train_z.shape[2], drop_prob=0.5)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_dir', type=str, default="C:\\Users\\ganga\\Github\\Generative-Models\\Project\\Data\\Dynamics\\", help='folder containing train_p_h_v.pkl and test_p_h_v.pkl')
    parser.add_argument('--out_dir', type=str, default="C:\\Users\\ganga\\Github\\Generative-Models\\Project\\Outputs\\Dynamics\\", help='output directory')
    parser.add_argument('--option', type=str, default="LSTM", help='Option to run (LSTM, FF)')
    parser.add_argument('--batch_size', type=int, default=16, help='mini batch size')
    parser.add_argument('--epoch', type=int, default=200, help='No. of epochs to train')
